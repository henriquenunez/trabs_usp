...
struct tPointStructure {
   ...
   tPointi v;
   bool delete;
   int interior; /*field for checking if the point belongs to interior of the hull*/
};
...
void   PrintPoints( void )
{
   ...
   printf("Points:\n");
   for( i = 0; i < n; i++ )
      printf("vnum=%3d, x=%4d, y=%4d, delete=%d, interior=%d\n", 
	     P[i].vnum, P[i].v[X], P[i].v[Y], P[i].delete, P[i].interior);
}
